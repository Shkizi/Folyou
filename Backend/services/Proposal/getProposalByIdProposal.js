...
function getProposalByIdProposal(req, res, next) {
   ...
   "GROUP BY Proposal_idProposal ) AS keywords ,(SELECT filename as `avatarUser`, User_idUser from Anexes) as avatar  " +
    ...
    " AND `User`.`idUser` = `Proposal`.`User_idUser` AND `User`.`idUser` = avatar.User_idUser AND `Proposal`.`idProposal` = ? "+
    "AND  keywords.Proposal_idProposal = idProposal "+
    "AND anexedfiles.Proposal_idProposal = idProposal "+
    "ORDER BY `Proposal`.`createdTimestamp` DESC LIMIT ? ; ",[parseInt(params.idProposal),parseInt(params.limit)] , function (rows, error) {
        console.log(params);
        if (!error) {
...
module.exports = getProposalByIdProposal;