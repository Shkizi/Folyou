...
function getTalentByIdUser(req, res, next) {
   ...
   "GROUP BY TalentArea_idTalentArea ) AS keywords,(SELECT filename as `avatarUser`, User_idUser from Anexes) as avatar " +
    ...
    " AND `User`.`idUser` = `TalentArea`.`User_idUser` AND `User`.`idUser` = avatar.User_idUser  AND `User`.`idUser` =  ? "+
    "AND  keywords.TalentArea_idTalentArea = idTalentArea "+
    "ORDER BY `TalentArea`.`timestamp` DESC LIMIT ? ; ",[parseInt(params.idUser),parseInt(params.limit)] , function (rows, error) {
        console.log(params);
        if (!error) {
...
module.exports = getTalentByIdUser;