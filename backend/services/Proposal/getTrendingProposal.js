...
function getTrendingProposal(req, res, next) {
    ...
    db.query("SELECT *, count(*) as `contage` FROM `Iteraction`,`Proposal`, `Category`, `User`, (SELECT Proposal_idProposal, GROUP_CONCAT(DISTINCT valueProposalKeywords) keywords FROM keyword WHERE Proposal_idProposal IS NOT NULL GROUP BY Proposal_idProposal ) AS keywords ,(SELECT filename as `avatarUser`, User_idUser from Anexes) as avatar  WHERE `Category`.`idCategory` =`Proposal`.`Category_idCategory` AND `User`.`idUser` = `Proposal`.`User_idUser` AND `User`.`idUser` = avatar.User_idUser AND  keywords.Proposal_idProposal = idProposal AND `Iteraction`.`Proposal_idProposal`= `Proposal`.`idProposal` GROUP BY `Proposal`.`idProposal` ORDER BY `contage`  DESC LIMIT 2 ;",[] , function (rows, error) {
        console.log(params);
        if (!error) {
